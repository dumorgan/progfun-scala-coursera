...
object GoogleVsApple {
  ...
  def compareFunc(tweet: Tweet)(keywords: List[String]): Boolean = keywords match {
    case List() => false
    case keyword :: kz =>
      if (keyword.contentEquals(tweet.text)) true
      else compareFunc(tweet)(kz)
  }

  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => compareFunc(tweet)(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => compareFunc(tweet)(apple))
